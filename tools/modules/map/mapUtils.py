...
def zoom_to_area(area, (width, height)):
    start =  MAP_MIN_ZOOM_LEVEL
    while start <= MAP_MAX_ZOOM_LEVEL:
        t1 = coord_to_tile((area[0],area[2],start))
        t2 = coord_to_tile((area[1],area[3],start))
        diff = (t2[0][0]-t1[0][0],t1[0][1]-t2[0][1])
        tile = ((t1[0][0]+(diff[0]/2),t1[0][1]-(diff[1]/2)),(TILES_WIDTH//2,TILES_HEIGHT//2))
        if width >= diff[0]*TILES_WIDTH and height >= diff[1]*TILES_HEIGHT:
            return (tile_to_coord(tile,start),tile)
        start += 1
